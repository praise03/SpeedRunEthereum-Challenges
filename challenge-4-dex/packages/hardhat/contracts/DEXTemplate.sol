...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        require(token.transferFrom(msg.sender, address(this), tokens), "DEX - init token transfer did not transact");
    ...
    function tokenToEth(uint256 tokenInput) public payable returns (uint256 ethOutput) {
        require(token.balanceOf(msg.sender) > 0, "Token balance must be greater than 0");
        uint256 ethReserve = address(this).balance.sub(msg.value);
        uint256 tokenReserve = token.balanceOf(address(this));
        uint256 ethOutput = price(tokenInput, tokenReserve, ethReserve);

        require(token.transferFrom(msg.sender, address(this), ethOutput), "tokenToEth(): reverted swap.");
